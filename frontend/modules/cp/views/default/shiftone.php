...
$this->title = "Tezkor tergov guruhi ro`yhati";
?>
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <?php foreach ($data as $key=>$item): ?>
                            <tr>
                                ...
                                <td><a href="<?= Yii::$app->urlManager->createUrl(['/cp/default/view','id'=>$item->id])?>"><?= $item->name ?></a></td>
                                <td><?= $item->username ?></td>
                                <td><?= $item->hudud ?></td>
                                <td><?= $item->pozivnoy ?></td>
                                <?php if($date == date('Y-m-d')){?><td><a class="btn btn-danger"
                                                                          href="<?= Yii::$app->urlManager->createUrl(['/cp/default/shiftremove', 'user_id' => $item->id, 'shift_id' => 1]) ?>"><span class="fa fa-trash"></span></a></td><?php }?>
                            </tr>
                        <?php endforeach;?>
                        </tbody>
                    </table>
...
<?php
$url = Yii::$app->urlManager->createUrl(['/cp/default/shiftone']);
$this->registerJs("
