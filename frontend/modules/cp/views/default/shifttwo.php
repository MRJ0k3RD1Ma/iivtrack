                        ...
                        <?php foreach ($data as $key=>$item): ?>
                            <tr>
                                ...
                                <?php if($date == date('Y-m-d')){?><td><a class="btn btn-danger"
                                            href="<?= Yii::$app->urlManager->createUrl(['/cp/default/shiftremove', 'user_id' => $item->id, 'shift_id' => 2]) ?>"><span class="fa fa-trash"></span></a></td><?php }?>
                            </tr>
                        <?php endforeach;?>
                        </tbody>
                    </table>
...
<?php
    $url = Yii::$app->urlManager->createUrl(['/cp/default/shifttwo']);
    $this->registerJs("
